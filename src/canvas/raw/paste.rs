...
impl RawCanvas {
    pub fn paste(&mut self, yank: &CanvasYank, origo: CanvasIndex) {
        for (index_yank, cell_yank) in &yank.cells {
            ...
            if yank.content_type.contains(ContentType::TEXT) {
                cell.character = cell_yank.character;
            }
            if yank.content_type.contains(ContentType::FG) {
                cell.fg = cell_yank.fg;
            }
            if yank.content_type.contains(ContentType::BG) {
                cell.bg = cell_yank.bg;
            }
            if yank.content_type.contains(ContentType::MODIFIERS) {
